...
//判断购物车表中是否已经存在该商品记录
$sql = "SELECT ctid FROM k_cart WHERE userid=$uid AND did=$did";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_assoc($result);
if($row){
  if($count == -1) //之前曾经购买过该商品，则更新购买数量加1
  {
    $sql = "update k_cart set dishCount=dishCount+1 where userid=$uid AND did=$did";
  }
  else //之前曾经购买过该商品，则更新购买数量为参数中的$count
  {
    $sql = "update k_cart set dishCount='$count' where userid=$uid AND did=$did";
  }
  ...
  $sql = "INSERT INTO k_cart VALUES(NULL,$uid, $did,1)";
  mysqli_query($conn,$sql);
  $output['code'] = 1;
  $output['msg'] = 'succ';
}
